...
pub async fn get_ledger_handler(
    ...
    let from = query.from.unwrap_or(chrono::NaiveDate::from_ymd_opt(2020, 1, 1).unwrap());
...
pub async fn close_period_handler(
    ...
    match service.close_period(&body.period).await {
        ...
        Err(AccountingServiceError::PeriodAlreadyClosed(p)) => {
            HttpResponse::Conflict().json(serde_json::json!({"error": format!("Period {p} already closed")}))
        }
        Err(e) => HttpResponse::InternalServerError().json(serde_json::json!({"error": e.to_string()})),
    }
}
...
pub async fn get_ecl_staging_handler(
    _query: web::Query<EclStagingQuery>,
) -> HttpResponse {
