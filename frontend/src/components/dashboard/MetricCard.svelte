...
>
  <div class="flex items-center justify-between">
    ...
    <span class="inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-medium {config.text}" data-testid="metric-card-status">
      <span class="h-2 w-2 rounded-full {config.dot}" aria-hidden="true"></span>
      {config.label}
    </span>
  </div>
  <div class="mt-3">
    <p class="text-3xl font-bold {config.text}" data-testid="metric-card-value">
